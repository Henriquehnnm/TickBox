...
public class TickBox
{
    ...
    public TickBox(string name, string description, DateTime startDate, DateTime endDate)
    {
    ...
    public async Task CreateChildAsync(string name, string description, Duration durarionTime)
    {
        ...
        await refreshData(Path.Join(globalPath, Parent.Name, "meta.json"), BoxType.ParentBox, "a"); // Gambiarra das braba desse a aqui, C# nao tem Option<>
    }

    public async Task CreateActionAsync(string name, string description, Duration durationTime, string content, string childName)
    {
        var real = GetChildByName(childName);
